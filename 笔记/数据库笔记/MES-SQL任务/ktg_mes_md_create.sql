...
-- 6.物料供应商关联表 （补缺失表）
-- ----------------------------
drop table if exists md_item_vendor;
create table md_item_vendor (
  ...
  vendor_code       varchar(64)     not null                   comment '供应商编码',
  vendor_name       varchar(255)    not null                   comment '供应商名称',
  vendor_type       varchar(64)     default ''                 comment '供应类型(主料/辅料/备选)',
  is_main_vendor    char(1)         default 'N' not null       comment '是否主供应商 Y/N',
  enable_flag       char(1)         default 'Y' not null       comment '是否启用',
  remark            varchar(500)    default ''                 comment '备注',
  create_by         varchar(64)     default ''                 comment '创建者',
  create_time       datetime                                   comment '创建时间',
  ...
  update_time       datetime                                   comment '更新时间',
  primary key (id),
  unique key uk_item_vendor (item_id, vendor_id)
) engine=innodb auto_increment=100 comment = '物料供应商关联表';
-- ----------------------------
-- 7、客户表
-- ----------------------------
